 ...
 * @template {keyof AbraXCUITestDriver.executeMethodMap} Script
...
function executeMethodExpectsParam(script, param) {
  ...
  /** @type {ReadonlyArray<string>|undefined} */
  let optional;
  const execMethodDef = AbraXCUITestDriver.executeMethodMap[script];
...
/**
 * @param {any} script
 * @returns {script is keyof AbraXCUITestDriver.executeMethodMap}
 */
function isExecuteMethod(script) {
  return script in AbraXCUITestDriver.executeMethodMap;
}

/**
 * Massages the arguments going into an execute method.
 * @param {keyof AbraXCUITestDriver.executeMethodMap} script
